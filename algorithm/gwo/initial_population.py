...
def create_initial_population(context, size: int, rng=None):
    """Build GWO wolves from greedy schedules and discrete coverage."""
    rng = rng or random.Random()
    size = max(3, int(size))
    tasks = [int(task) for task in context.task_order]
    if not tasks:
        raise ValueError("GWO task order is empty")
    ...
    if not greedy:
        raise RuntimeError("GWO greedy initialization returned no solution")
    ...
    if len(population) != size:
        raise RuntimeError(
            f"GWO created {len(population)} unique wolves; expected {size}"
        )
    return population
