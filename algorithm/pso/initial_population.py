...
def create_initial_population(context, size: int, rng=None):
    """Build PSO particles from greedy schedules and discrete coverage."""
    rng = rng or random.Random()
    size = max(3, int(size))
    tasks = [int(task) for task in context.task_order]
    if not tasks:
        raise ValueError("PSO task order is empty")
    ...
    if not greedy:
        raise RuntimeError("PSO greedy initialization returned no solution")
    ...
    if len(population) != size:
        raise RuntimeError(
            f"PSO created {len(population)} unique particles; expected {size}"
        )
    return population
